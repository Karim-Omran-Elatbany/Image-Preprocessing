function [newImg] = maxfilter(oldImg,s,n)
[R ,C, L]=size(oldImg);
h=R;
w=C;
k=(s-1)/2;
...
% GRAY
    ...
    elseif(n==2)
     ...
     for i=1:k
            filter(1:R,i)=filter(:,1+k);
     end
         for i=1:k
            filter(1:R,C-k+i)=filter(:,end-k);
         end
